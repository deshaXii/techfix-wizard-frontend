  ...
  return (
    <div className="ff-reveal flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
      <div>
        ...
        {desc ? (
          <p className="mt-3 text-sm sm:text-base text-ink/70 max-w-2xl">
            {desc}
          </p>
        ) : null}
      </div>
...
function ScreenshotCard({ t }) {
  return (
    <div className="relative">
      ...
      <div
        className="absolute -bottom-14 -left-10 w-64 h-64 bg-primary/15 blur-3xl rounded-full ff-blob"
      ...
      <div className="relative bg-white border border-primary-100 rounded-3xl shadow-soft p-4 sm:p-6 overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 1px 1px, rgba(31,115,201,0.35) 1px, transparent 0)",
        ...
        <div className="absolute left-6 bottom-6 hidden sm:flex gap-2">
          <div className="bg-white/90 backdrop-blur border border-primary-100 rounded-2xl shadow-soft px-3 py-2">
            <div className="text-[11px] font-semibold text-ink/70">
              {t("home.metricAvgLabel")}
            </div>
            <div className="text-sm font-extrabold text-ink">
              {t("home.metricAvgValue")}
            </div>
          </div>
          <div className="bg-white/90 backdrop-blur border border-primary-100 rounded-2xl shadow-soft px-3 py-2">
            <div className="text-[11px] font-semibold text-ink/70">
              {t("home.metricDiscountLabel")}
            </div>
            <div className="text-sm font-extrabold text-primary">
              {t("home.metricDiscountValue")}
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-16">
      ...
      <section className="relative">
        <div className="ff-reveal relative overflow-hidden rounded-3xl border border-primary-100 bg-white shadow-soft">
          ...
          <div
            className="absolute -bottom-20 -right-24 w-80 h-80 bg-primary/15 blur-3xl rounded-full ff-blob"
            ...
            <div>
              <div className="flex flex-wrap gap-2">
                <DeviceChip
                  ...
                  label={t("wizard.deviceTypes.phone")}
                />
                <DeviceChip
                  icon={<Icon.Tablet className="h-4 w-4" />}
                  label={t("wizard.deviceTypes.tablet")}
                />
                <DeviceChip
                  icon={<Icon.Laptop className="h-4 w-4" />}
                  label={t("wizard.deviceTypes.laptop")}
                />
              </div>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3 max-w-md">
                {[1, 2, 3].map((n) => (
                  ...
                  >
                    <div
                      className={`h-10 w-10 rounded-2xl flex items-center justify-center ${n === 1 ? "bg-primary text-white" : "bg-white text-ink/60 border border-primary-100"} transition group-hover:scale-[1.03]`}
                    >
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="ff-reveal group bg-white border border-primary-100 rounded-3xl shadow-soft p-6 flex items-center gap-5 hover:shadow-lg hover:-translate-y-1 transition">
            <img
              src="/illustrations/device-phone.svg"
              alt="Phone"
              className="w-20 h-20"
              loading="lazy"
            />
            <div>
              <div className="text-sm font-extrabold text-ink">
                {t("wizard.deviceTypes.phone")}
              </div>
              <div className="mt-1 text-xs text-ink/70">
                {t("home.supportPhoneDesc")}
              </div>
            </div>
          </div>
          <div className="ff-reveal group bg-white border border-primary-100 rounded-3xl shadow-soft p-6 flex items-center gap-5 hover:shadow-lg hover:-translate-y-1 transition">
            <img
              src="/illustrations/device-tablet.svg"
              alt="Tablet"
              className="w-20 h-20"
              loading="lazy"
            />
            <div>
              <div className="text-sm font-extrabold text-ink">
                {t("wizard.deviceTypes.tablet")}
              </div>
              <div className="mt-1 text-xs text-ink/70">
                {t("home.supportTabletDesc")}
              </div>
            </div>
          </div>
          <div className="ff-reveal group bg-white border border-primary-100 rounded-3xl shadow-soft p-6 flex items-center gap-5 hover:shadow-lg hover:-translate-y-1 transition">
            <img
              src="/illustrations/device-laptop.svg"
              alt="Laptop"
              className="w-20 h-20"
              loading="lazy"
            />
            <div>
              <div className="text-sm font-extrabold text-ink">
                {t("wizard.deviceTypes.laptop")}
              </div>
              <div className="mt-1 text-xs text-ink/70">
                {t("home.supportLaptopDesc")}
              </div>
            </div>
          </div>
        </div>
          ...
          {[
            {
              ...
              quote:
                "“The wizard was super clear. I got a quote instantly and the repair was done the same day.”",
            },
            {
              name: "Sara",
              title: "Laptop battery",
              quote:
                "“No hidden costs. I knew the price before I went to the shop.”",
            },
            {
              name: "Omar",
              title: "Charging port",
              quote:
                "“Fast, clean, professional. The nearby shops section helped me find a place around me.”",
            },
          ].map((t) => (
            <div
              key={t.name}
              className="ff-reveal bg-white border border-primary-100 rounded-3xl shadow-soft p-6"
            >
          ...
          <div className="relative grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <div className="text-sm font-bold text-ink">
                {t("home.previewCtaTitle")}
              </div>
              <div className="mt-2 text-sm text-ink/70">
                {t("home.previewCtaDesc")}
              </div>
      ...
      <section className="ff-reveal">
        <div className="relative overflow-hidden bg-primary text-white rounded-3xl shadow-soft p-8 sm:p-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          ...
          <div className="relative">
            ...
            <div className="mt-2 text-sm text-white/85 max-w-2xl">
              {t("home.ctaDesc")}
            </div>
          </div>
          <Link
